...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const [activeField, setActiveField] = useState(null); // 'name', 'email', or null
    ...
    <div className="flex justify-center items-end h-screen w-screen bg-white overflow-hidden font-sans select-none">
      <div className="relative w-full max-w-[400px] h-full flex flex-col">
        ...
        >
          <div className="text-[4rem] font-[900] uppercase flex gap-[0.05em] text-black transition-colors duration-500">
            ...
            {letters.map((char, i) => (
              <span 
                key={i} 
                className={`inline-block min-w-[0.6em] text-center transition-all
                  ${!isLoaded ? 'animate-font-cycle' : 'font-sans'}
                `}
                style={{ 
        ...
        <div 
          ...
          `}
        >
          {!isLogin && (<div className={`relative h-full flex items-center transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden
              ${activeField === 'email' ? 'flex-[0_0_48px]' : 'flex-1'}
            `}>
              <User 
                size={20}
                className={`absolute left-4 transition-all duration-300 ease-[cubic-bezier(0.68,-0.6,0.32,1.6)] z-20 
                  ${activeField === 'email' ? 'left-1/2 -translate-x-1/2 scale-125 text-[#906efc] cursor-pointer' : 'text-gray-400'}
                `}
              ...
              <input 
                ...
                onBlur={handleBlur}
                className={`w-full h-full pl-[45px] pr-5 rounded-[50px] border border-gray-200 outline-none text-[0.85rem] bg-white focus:border-[#906efc] transition-opacity duration-300
                  ${activeField === 'email' ? 'opacity-0 pointer-events-none' : 'opacity-100'}
                `}
              />
            </div>)}

            <div className={`relative h-full flex items-center transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden
              ${activeField === 'name' ? 'flex-[0_0_48px]' : 'flex-1'}
            `}>
              <Mail 
                size={20}
                className={`absolute left-4 transition-all duration-300 ease-[cubic-bezier(0.68,-0.6,0.32,1.6)] z-20
                  ${activeField === 'name' ? 'left-1/2 -translate-x-1/2 scale-125 text-[#906efc] cursor-pointer' : 'text-gray-400'}
                `}
              ...
              <input 
                ...
                onBlur={handleBlur}
                className={`w-full h-full pl-[45px] pr-5 rounded-[50px] border border-gray-200 outline-none text-[0.85rem] bg-white focus:border-[#906efc] transition-opacity duration-300
                  ${activeField === 'name' ? 'opacity-0 pointer-events-none' : 'opacity-100'}
                `}
              />
            </div>
          ...
          <button onClick={handleGoogleSignIn} className="w-full py-3.5 rounded-[50px] font-semibold text-[0.9rem] cursor-pointer border-none text-white bg-[linear-gradient(135deg,#5b86e5_0%,#906efc_100%)] flex items-center justify-center gap-2.5 shadow-[0_4px_10px_rgba(91,134,229,0.15)] transition-all duration-400 ease-[cubic-bezier(0.68,-0.6,0.32,1.6)] hover:brightness-[1.08] hover:-translate-y-1 hover:scale-[1.03] hover:shadow-[0_10px_20px_rgba(91,134,229,0.25)] active:scale-[0.92]">
           ...
           <button onClick={() => setIsLogin(!isLogin)} className="text-xs text-center text-gray-500 hover:text-black">
