...
const QuickTools: React.FC<QuickToolsProps> = ({
    ...
    const toggleTool = (name: string) => {
        ...
        if (isTogglingOff) {
            setCurrentTool({ name: null, elementId: null });
        } else {
            setCurrentTool({ name, elementId: selectedElement.id });
        }
    };
    ...
    const renderActiveTool = () => {
        ...
        switch (activeToolName) {
            ...
            case 'fontSize':
                const currentSize = selectedElement.style.fontSize || 16;
                return (
                    <div className="flex items-center gap-2 text-white">
                        <button onClick={() => handleStyleChange('fontSize', currentSize - 1)}>{Icons.Minus && <Icons.Minus className="w-5 h-5" />}</button>
                        <input type="range" min="8" max="128" value={currentSize} onChange={e => handleStyleChange('fontSize', parseInt(e.target.value))} className="w-full" />
                        <button onClick={() => handleStyleChange('fontSize', currentSize + 1)}>{Icons.Plus && <Icons.Plus className="w-5 h-5" />}</button>
                        <span className="text-xs font-bold w-12 text-center">{currentSize}px</span>
                    </div>
                );
            case 'color':
                const prop = selectedElement.type === 'text' || selectedElement.type === 'icon' ? 'color' : 'backgroundColor';
                return <CustomColorPicker color={selectedElement.style[prop]} onChange={color => handleStyleChange(prop, color)} themeColors={themeColors} />;
            case 'format':
                return (
                    <div className="flex items-center gap-2 text-white">
                        <button onClick={() => handleStyleChange('textAlign', 'left')} className={selectedElement.style.textAlign === 'left' ? 'text-lime-400' : ''}>{Icons.AlignLeft && <Icons.AlignLeft className="w-5 h-5" />}</button>
                        <button onClick={() => handleStyleChange('textAlign', 'center')} className={selectedElement.style.textAlign === 'center' ? 'text-lime-400' : ''}>{Icons.AlignCenter && <Icons.AlignCenter className="w-5 h-5" />}</button>
                        <button onClick={() => handleStyleChange('textAlign', 'right')} className={selectedElement.style.textAlign === 'right' ? 'text-lime-400' : ''}>{Icons.AlignRight && <Icons.AlignRight className="w-5 h-5" />}</button>
                    </div>
                );
            case 'effects':
                return (
                    <div className="grid grid-cols-4 gap-2 p-2 text-white">
                        <button onClick={() => onApplyEffect('none')} className="flex flex-col items-center gap-1 text-white/70 hover:text-white">{Icons.X && <Icons.X className="w-5 h-5" />}<span className="text-xs">None</span></button>
                        <button onClick={() => onApplyEffect('grayscale')} className="flex flex-col items-center gap-1 text-white/70 hover:text-white">{Icons.Palette && <Icons.Palette className="w-5 h-5" />}<span className="text-xs">Grayscale</span></button>
                        <button onClick={() => onApplyEffect('sepia')} className="flex flex-col items-center gap-1 text-white/70 hover:text-white">{Icons.Palette && <Icons.Palette className="w-5 h-5" />}<span className="text-xs">Sepia</span></button>
                        <button onClick={() => onApplyEffect('invert')} className="flex flex-col items-center gap-1 text-white/70 hover:text-white">{Icons.Palette && <Icons.Palette className="w-5 h-5" />}<span className="text-xs">Invert</span></button>
                        <button onClick={() => onApplyEffect('motion-blur')} className="flex flex-col items-center gap-1 text-white/70 hover:text-white">{Icons.Wind && <Icons.Wind className="w-5 h-5" />}<span className="text-xs">Blur</span></button>
                    </div>
                );
            default: return null;
        }
    ...
    const getToolsForElement = () => {
        if (!selectedElement) {
            return (
                <>
                    <button onClick={onOpenSidebar} className="flex flex-col items-center gap-1 text-lime-400 p-2 min-w-[50px]">
                        {Icons.Plus && <Icons.Plus className="w-5 h-5" />}
                        <span className="text-[10px] font-bold uppercase">Add</span>
                    </button>
                    <div className="flex gap-2 items-center">
                        {themeColors.slice(0, 3).map(c => <button key={c} onClick={() => updatePage({ background: c })} className={'w-8 h-8 rounded-full border border-white/20'} style={{ backgroundColor: c }} />)}
                    </div>
                </>
            );
        }
        ...
        switch (selectedElement.type) {
            case 'text':
                return (
                    <>
                        <button onClick={() => toggleTool('text')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'text' ? 'text-lime-400' : 'text-white/60'}`}><span className="text-[10px] font-bold uppercase">Edit</span></button>
                        <button onClick={() => toggleTool('font')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'font' ? 'text-lime-400' : 'text-white/60'}`}><span className="text-[10px] font-bold uppercase">Font</span></button>
                        <button onClick={() => toggleTool('fontSize')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'fontSize' ? 'text-lime-400' : 'text-white/60'}`}><span className="text-[10px] font-bold uppercase">Size</span></button>
                        <button onClick={() => toggleTool('color')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'color' ? 'text-lime-400' : 'text-white/60'}`}><span className="text-[10px] font-bold uppercase">Color</span></button>
                        <button onClick={() => toggleTool('format')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'format' ? 'text-lime-400' : 'text-white/60'}`}><span className="text-[10px] font-bold uppercase">Format</span></button>
                    </>
                );
            case 'image':
                return (
                    <>
                        ...
                        <button onClick={() => imageReplaceInputRef.current?.click()} className="flex flex-col items-center gap-1 text-white/60 p-2 min-w-[50px]">{Icons.Image && <Icons.Image className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Replace</span></button>
                        <button onClick={() => toggleTool('effects')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'effects' ? 'text-lime-400' : 'text-white/60'}`}>{Icons.Wand2 && <Icons.Wand2 className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Effects</span></button>
                        <button onClick={() => onOpenSidebar()} className="flex flex-col items-center gap-1 text-white/60 p-2 min-w-[50px]">{Icons.Layers && <Icons.Layers className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Layer</span></button>
                    </>
                );
            case 'shape':
                return (
                    <>
                         <button onClick={() => toggleTool('color')} className={`flex flex-col items-center gap-1 p-2 min-w-[50px] ${activeToolName === 'color' ? 'text-lime-400' : 'text-white/60'}`}>{Icons.Palette && <Icons.Palette className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Color</span></button>
                         <button onClick={() => onOpenSidebar()} className="flex flex-col items-center gap-1 text-white/60 p-2 min-w-[50px]">{Icons.Layers && <Icons.Layers className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Style</span></button>
                         <button onClick={() => onReorder(selectedElement.id, 'up')} className="flex flex-col items-center gap-1 text-white/60 p-2 min-w-[50px]">{Icons.ChevronUp && <Icons.ChevronUp className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Layer</span></button>
                    </>
                );
            default:
                return (
                    <>
                        <button onClick={() => onOpenSidebar()} className="flex flex-col items-center gap-1 text-lime-400 p-2 min-w-[50px]">{Icons.Sparkles && <Icons.Sparkles className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Style</span></button>
                        <button onClick={() => onOpenSidebar()} className="flex flex-col items-center gap-1 text-white/60 p-2 min-w-[50px]">{Icons.Layers && <Icons.Layers className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Layer</span></button>
                        {selectedElement && <button onClick={() => deleteElement(selectedElement.id)} className="flex flex-col items-center gap-1 text-red-400 p-2 min-w-[50px]">{Icons.Trash2 && <Icons.Trash2 className="w-5 h-5" />}<span className="text-[10px] font-bold uppercase">Delete</span></button>}
                    </>
                );
        }
    };
        ...
        <div className="mx-4 mb-4 bg-zinc-900/95 backdrop-blur-lg border border-white/10 rounded-2xl shadow-2xl p-2">
            ...
            <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex-shrink-0 flex items-center gap-1">
                    {selectedElement && <button onClick={() => onReorder(selectedElement.id, 'down')} className="bg-white/5 p-3 rounded-full">{Icons.ChevronDown && <Icons.ChevronDown className="w-5 h-5 text-white" />}</button>}
                    {selectedElement && <button onClick={() => onReorder(selectedElement.id, 'up')} className="bg-white/5 p-3 rounded-full">{Icons.ChevronUp && <Icons.ChevronUp className="w-5 h-5 text-white" />}</button>}
                    <button onClick={onOpenSidebar} className="bg-white/5 p-3 rounded-full">{Icons.Menu && <Icons.Menu className="w-5 h-5 text-white" />}</button>
                </div>
            </div>
        </div>
    );
