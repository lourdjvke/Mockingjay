...
const Section: React.FC<SectionProps> = ({ title, id, icon, children, isOpen, onToggle, isMobile }) => (
  <motion.div 
    ...
    transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1], delay: isMobile ? 0.1 : 0 }}
  >
    <button
      onClick={() => onToggle(id)}
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleDragEnd: OnDragEndResponder = (result) => {
    ...
    const totalLayers = currentPage.elements.length;
    onLayerReorder(totalLayers - 1 - startIndex, totalLayers - 1 - endIndex);
    triggerHaptic(10);
  };
  ...
  const brandDnaButtonContent = () => {
    if (isLoading) {
        return <Icons.Loader2 className="w-5 h-5 animate-spin" />;
    }
    return brandData ? 'Use Brand DNA' : 'Create Brand DNA';
  };
    ...
    <div className={`${isMobile ? 'w-full' : 'w-[380px]'} h-full bg-white ${isMobile ? '' : 'border-l'} border-gray-200 flex flex-col select-none overflow-hidden`}>
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence>
            {!isLoading && (
            <>
            <Section title="Design History" id="history" icon={<Icons.History className="w-4 h-4" />} isOpen={openSections.history} onToggle={toggleSection} isMobile={isMobile}>
            ...
            <div className="px-5 py-4 border-b border-gray-200">
               ...
               <div className="flex gap-2 flex-wrap">
                ...
                <button
                    ...
                    className="mt-4 w-full h-10 rounded-lg text-xs font-bold uppercase flex items-center justify-center gap-2 border transition-all duration-150 ease-out active:scale-[0.98] transform relative overflow-hidden"
                    style={{
                ...
                >
                  ...
                  <span className="relative z-10">{brandDnaButtonContent()}</span>
                </button>
            </div>

            <Section title="Media" id="media" icon={<Icons.ImageIcon className="w-4 h-4" />} isOpen={openSections.media} onToggle={toggleSection} isMobile={isMobile}>
            ...
            <Section title="Assets" id="image" icon={<Icons.Plus className="w-4 h-4" />} isOpen={openSections.image} onToggle={toggleSection} isMobile={isMobile}>
            ...
            {selectedElement && (
              <>
                <Section title="Element Edit" id="text" icon={<Icons.Type className="w-4 h-4" />} isOpen={openSections.text} onToggle={toggleSection} isMobile={isMobile}>
                   <div className="space-y-4">
                ...
                <Section title="Appearance" id="appearance" icon={<Icons.Sparkles className="w-4 h-4" />} isOpen={openSections.appearance} onToggle={toggleSection} isMobile={isMobile}>
                ...
                <Section title="Layers" id="layers" icon={<Icons.Layout className="w-4 h-4" />} isOpen={openSections.layers} onToggle={toggleSection} isMobile={isMobile}>
                    <DragDropContext onDragEnd={handleDragEnd}>
                        <Droppable droppableId="layers">
                            {(provided) => (
                                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2">
                                    {currentPage?.elements.slice().reverse().map((el, index) => (
         ...
         {isLoading && isMobile && (
            <div className="flex justify-center items-center h-full">
                <Icons.Loader2 className="w-8 h-8 animate-spin text-gray-300" />
            </div>
        )}
      </div>
    </div>
